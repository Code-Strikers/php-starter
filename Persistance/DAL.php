<?php
	Class DAL{
		...
		public function getPersonnage(){
			...
			$param = array('1' => array(1, PDO::PARAM_INT));
			$statement = $BD->query($req, $param);
			$personnages = $BD->getResults($statement);

			foreach ($personnages as $personnage){
				$tPersonnages[] = new Personnage($personnage[0], $personnage[1], $personnage[2], $personnage[3], $personnage[4]);
			}
		...
		public function ajouterPersonnage($nom, $origine,$detail,$photo){
			...
			$req = 'INSERT INTO personnage(nom, origine, detail, cheminPhoto) VALUES(?, ?, ?, ?)';
			$param = array('1' => array($nom, PDO::PARAM_STR),
					'2' => array($origine, PDO::PARAM_STR),
					'3' => array($detail, PDO::PARAM_STR),
					'4' => array($photo, PDO::PARAM_STR));
		...
		public function getDetailsPersonnage($idPersonnage){

			$BD = ClassBD::getInstance();

			$req = "SELECT * FROM personnage WHERE idPersonnage= ?";
			$param = array('1' => array($idPersonnage, PDO::PARAM_INT));
			$statement = $BD->query($req, $param);
			$personnageCourant = $BD->getResults($statement);
			$tableauDetails['Personnage'] = new Personnage($personnageCourant[0][0], $personnageCourant[0][1], $personnageCourant[0][2],
					$personnageCourant[0][3], $personnageCourant[0][4]);


			/*$req3 = 'SELECT * FROM commentaire WHERE idTitre = ?';
			$param3 = array('1' => array($idTitre, PDO::PARAM_INT));
			$statement3 = $BD->query($req3, $param3);
			$commentaires = $BD->getResults($statement3);
			if(!empty($commentaires)){
				foreach($commentaires as $commentaire){
					$tableauCommentaires[] = new Commentaire($commentaire[0], $commentaire[1], $commentaire[2]);
				}
		...
		/* Function permettant de tester si l'id d'un titre existe afin de sécuriser les id d'un Personnage passé par URL*/
		public function isIdPersonnageExistant($id){

			$BD = ClassBD::getInstance();

			$req = 'SELECT COUNT(*) FROM personnage WHERE idPersonnage = ?';
			$param = array('1' => array($id, PDO::PARAM_INT));
			$statement = $BD->query($req, $param);
			$isIdPersonnageExistant = $BD->getResults($statement);
			if($isIdPersonnageExistant[0][0] == 1){
				$BD::destructInstance();
				return true;
			} else { $BD::destructInstance(); return false; }
		}
		...
		public function supprimerPersonnage($idArticle){
			...
			$req2 = 'DELETE FROM personnage WHERE idPersonnage = ?';
			$param2 = array('1' => array($idArticle, PDO::PARAM_INT));
			$statement2 = $BD->query($req2, $param2);
		...
		/* Function qui permet de modifier un titre dont l'id a été passé en paramètre */
		public function modifierPersonnage($idPersonnage,$nom, $origine,$detail,$photo){

			$BD = ClassBD::getInstance();

			$req = 'UPDATE personnage SET nom = ?, origine = ?, detail = ?, cheminPhoto = ? WHERE idPersonnage = ?';
			$param = array('1' => array($nom, PDO::PARAM_STR),
					'2' => array($origine, PDO::PARAM_STR),
					'3' => array($detail, PDO::PARAM_STR),
					'4' => array($photo, PDO::PARAM_STR),
					'5'=> array($idPersonnage, PDO::PARAM_INT));
